...
namespace aslam {
  ...
  namespace internal {
    ...
    /** The class PredictableErrorTermFs implements an error term for an Imu sensor acceleration measurement
  ...
  template <int D, typename PredictionExpression = backend::VectorExpression<D>>
    ...
    public:
      ...
      const Input& getMeasurement() const { return _measurement; }
      /// Returns the covariance of the measurement
      const Covariance& getCovariance() const { return _cov; }
      /// Returns the covariance of the measurement
      Output getPrediction() const { return _measurementPredictionExpression.toValue(); }
    ...
    protected:
      ...
      virtual void evaluateJacobiansImplementation(aslam::backend::JacobianContainer& jacobians){
